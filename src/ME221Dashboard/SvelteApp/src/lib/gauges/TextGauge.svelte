...
<div class="relative flex h-full w-full flex-col items-center justify-center gap-0.5">
  ...
  {/if}
  {#if gauge.showValue}
    <span class="font-bold leading-tight" style="color: {gauge.textColor}; font-size: {valueSize}px; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);">
      {gauge.formattedValue}
    </span>
  {/if}
  {#if gauge.showUnit}
    <span class="leading-tight" style="color: {gauge.textColor}; font-size: {unitSize}px; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);">
      {gauge.unit}
    </span>
  {/if}
  {#if gauge.showName}
    <span class="max-w-full truncate leading-tight" style="color: {gauge.textColor}; font-size: {nameSize}px; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);">
      {gauge.name}
    </span>
  {/if}
</div>
